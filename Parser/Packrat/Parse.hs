...
instance Derivs d => Monad (Parser d) where 
	(Parser p1) >>= f = Parser parse where   
                parse dvs=first (p1 dvs)
                first(Parsed val rem err)=let Parser p2 = f val in second err (p2 rem)
                first (NoParse err)=NoParse err
                second e1 (Parsed v r e)=Parsed v r(joinErrors e1 e)
                second e1(NoParse e)=NoParse(joinErrors e1 e)
...
(Parser p1) <|> (Parser p2) = Parser parse where 
    parse dvs=first dvs (p1 dvs)
    first dvs(result@(Parsed v r e))=result
    first dvs(NoParse e)=second e (p2 dvs)
    second e1 (Parsed v r e)        = Parsed v r (joinErrors e1 e)
    second e1 (NoParse e)           = NoParse (joinErrors e1 e)
