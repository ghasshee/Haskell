...
happyExpList = Happy_Data_Array.listArray (0,44) ([20928,128,1024,0,4096,5,16384,0,49152,81,256,0,256,128,0,512,10464,20928,0,2048,4096,7168,14341,10,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parse","Tm","AppTm","ATm","lam","pi","x","'.'","ty","':'","'('","')'","univ","i","%eof"]
        bit_start = st * 17
        bit_end = (st + 1) * 17
...
happyReduction_1 ((HappyAbsSyn4  happy_var_6) `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn4
		 (\c -> Lam happy_var_2 (happy_var_4 c)(happy_var_6(addVar c happy_var_2))
	) `HappyStk` happyRest
...
happyReduction_2 ((HappyAbsSyn4  happy_var_6) `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn4
		 (\c -> Pi  happy_var_2 (happy_var_4 c)(happy_var_6(addVar c happy_var_2))
	) `HappyStk` happyRest
...
happyReduction_5 (HappyAbsSyn6  happy_var_2)
	(HappyAbsSyn5  happy_var_1)
	 =  HappyAbsSyn5
		 (\c -> App (happy_var_1 c)(happy_var_2 c)
	)
...
happyReduction_7 (HappyTerminal (LCID happy_var_1))
	 =  HappyAbsSyn6
		 (\c -> Var $ getVar c happy_var_1
	)
happyReduction_7 _  = notHappyAtAll 

happyReduce_8 = happySpecReduce_2  6 happyReduction_8
happyReduction_8 (HappyTerminal (INT happy_var_2))
	_
	 =  HappyAbsSyn6
		 (\c -> Univ happy_var_2
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	LAMBDA -> cont 7;
	PI -> cont 8;
	LCID happy_dollar_dollar -> cont 9;
	DOT -> cont 10;
	UCID happy_dollar_dollar -> cont 11;
	COLON -> cont 12;
	LPAREN -> cont 13;
	RPAREN -> cont 14;
	UNIV -> cont 15;
	INT happy_dollar_dollar -> cont 16;
	_ -> happyError' ((tk:tks), [])
	}
